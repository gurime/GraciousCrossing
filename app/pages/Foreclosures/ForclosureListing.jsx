...
const querySnapshot = await getDocs(collection(db, "Fore closures"));
const data = [];
...
export default function ForclosureListings() {
    ...
    const commentsRef = collection(db, 'Fore closures');
    ...
    const db = getFirestore();
    const commentDoc = await getDoc(doc(db, 'Fore closures', postId));
    if (commentDoc.exists()) {
    await deleteDoc(doc(db, 'Fore closures', postId));
...
<div className='sm-houlo' style={{fontSize:'11px' }}>{blog.propertyType}</div>
