...
const querySnapshot = await getDocs(collection(db, "Student Housing"));
const data = [];
...
export default function StudentListings() {
    ...
    const commentsRef = collection(db, 'Student Housing');
    ...
    const db = getFirestore();
    const commentDoc = await getDoc(doc(db, 'Student Housing', postId));
    if (commentDoc.exists()) {
    await deleteDoc(doc(db, 'Student Housing', postId));
